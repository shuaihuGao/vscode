...
class HashTable {
	...
	private _hashFunc(tokenTypeIndex: number, tokenModifierSet: number, languageId: number): number {
		const n = ((((tokenTypeIndex << 5) - tokenTypeIndex) + tokenModifierSet) << 5 + languageId);   // n = tokenTypeIndex * 31 + tokenModifierSet
		return ((((n << 5) - n) + languageId) | 0) % this._currentLength;  // n * 31 + languageId, keep as int32
	}
